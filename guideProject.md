# GitHub Repo Summarizer API — Task Spec
...
Return an appropriate HTTP status code and:
...
LLM API key is configured via the NEBIUS_API_KEY environment variable (we will provide our own key when testing). Do not hardcode API keys.

In case you are using an alternative LLM provider, you can use corresponding environment variable name, for example OPENAI_API_KEY, ANTHROPIC_API_KEY etc.
